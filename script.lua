...
-- WEBHOOK DE DISCORD (Preconfigurado)
...
-- IDs de los lugares
local LOBBY_PLACE_ID = 6872265039
...
local function getPlaceStatus(placeId)
    if placeId == LOBBY_PLACE_ID then
        return "Lobby"
    elseif placeId == GAME_PLACE_ID then
        return "In-Game"
    else
        return "Unknown"
    end
...
-- ScreenGui principal (solo crear si estamos en el juego)
...
-- Loop principal
if currentPlaceId == GAME_PLACE_ID then
    print("✅ Bridge Duels AutoFarm v1.1 Iniciado")
    print("🎮 Place ID:", currentPlaceId)
    print("👤 Usuario:", player.Name)
    print("🏆 Wins actuales:", totalWins)
    
    while true do
        if game.PlaceId ~= GAME_PLACE_ID then
            print("⚠️ Ya no estamos en el juego, deteniendo el farm")
            break
        end
        ...
        if TimerLabel then
            TimerLabel.Text = "⚡ Ejecutando Script..."
            if TimerStroke then
        ...
        if touchdownSuccess then
            ...
            print("🎉 Victoria #" .. totalWins .. " registrada!")
        end
        ...
        print("🔄 Teletransportando de vuelta al juego...")
...
elseif currentPlaceId == LOBBY_PLACE_ID then
    print("⏸️ Script en espera - Estás en el Lobby")
    print("🎮 Esperando a que entres al juego...")
    
    while game.PlaceId == LOBBY_PLACE_ID do
        task.wait(5)
    end
    
    print("✅ Detectado cambio al juego, reiniciando script...")
    print("🔄 Vuelve a ejecutar el script cuando estés en el juego")
else
